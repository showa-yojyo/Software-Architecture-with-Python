...
def mandelbrot_main(w, h, max_iterations=1000, output='mandelbrot_celery.png'):
    ...
    job = group([mandelbrot_calc_row.s(y, w, h, max_iterations)
                 for y in range(h)])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-W', '--width', help='Width of the image', type=int, default=640)
    parser.add_argument(
        '-H', '--height', help='Height of the image', type=int, default=480)
    parser.add_argument(
        '-n', '--niter', help='Number of iterations', type=int, default=1000)
    parser.add_argument(
        '-o', '--output', help='Name of output image file', default='mandelbrot_celery.png')
